...
struct ContentDailyRow: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                // 1. TOPIC — lo más importante, arriba del todo
                Text(post.topic)
                    .font(.subheadline.weight(.semibold))
                    .lineLimit(2)
                    .foregroundColor(.primary)

                // 2. Plataformas + badge de estado prominente
                HStack(spacing: 6) {
                    platformBadge("LI", available: post.linkedinPost != nil,  published: post.publishedTo.contains("linkedin"),  color: Color(red: 0.04, green: 0.4, blue: 0.76))
                    platformBadge("IG", available: post.instagramPost != nil, published: post.publishedTo.contains("instagram"), color: .pink)
                    platformBadge("X",  available: post.twitterPost != nil,   published: post.publishedTo.contains("twitter"),   color: Color.primary)
                    platformBadge("TK", available: post.tiktokScript != nil,  published: post.publishedTo.contains("tiktok"),    color: .purple)

                    Spacer()

                    // Estado como badge con fondo de color — ya no es texto diminuto
                ...
                // 3. Fecha + origen IA — info secundaria al fondo
                HStack(spacing: 4) {
                    Image(systemName: "sparkles")
                        .font(.caption2)
                        .foregroundColor(.purple)
    ...
    private func platformBadge(_ label: String, available: Bool, published: Bool, color: Color) -> some View {
        Text(label)
            ...
            .padding(.vertical, 3)
            .background(published ? color : (available ? color.opacity(0.12) : Color.gray.opacity(0.08)))
            .foregroundColor(published ? .white : (available ? color : Color(.tertiaryLabel)))
            .cornerRadius(5)
    }
}
